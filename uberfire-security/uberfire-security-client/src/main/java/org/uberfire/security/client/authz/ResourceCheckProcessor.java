...
import org.uberfire.security.annnotations.ResourceCheck;
...
public class ResourceCheckProcessor extends IOCDecoratorExtension<ResourceCheck> {
    ...
    public void generateDecorator(final Decorable decorable, final FactoryController controller) {
        ...
        if (!metaMethod.getReturnType().getName().equals("void")) {
            throw new RuntimeException("The @SecuredAction annotated method \"" +
        ...
        // Global action check
            ...
            if (resourceType == null || resourceType.trim().length() == 0) {
                throw new RuntimeException("The @SecuredAction parameter named \"type\" is missing " +
    ...
    public boolean implementsResource(MetaClass metaClass) {
        for (MetaClass iface : metaClass.getInterfaces()) {
            if (iface.asClass().equals(Resource.class)) {
                return true;
            }
        }
        return false;
